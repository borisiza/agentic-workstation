...
- source_spec: `skills/implementation-artifacts/spec-4-2-make-config-sourcing-consistent-across-the-scripts.md`
  summary: Removing `2>/dev/null` from `resolve_and_validate_role`'s config sourcing (to match `doctor.sh`'s already-shipped form, per this story's AC) means a malformed `config/local.env` can now leak literal file content on stderr for some syntax-error shapes (empirically confirmed: a bad parameter expansion like `${TOKEN!!!bad}` reproduces `${TOKEN!!!bad}: bad substitution` verbatim), conflicting with the repo's "never print key material/tokens/tailnet IPs" convention enforced elsewhere; separately, a config that sources successfully (rc=0) but emits its own stderr chatter now leaks that noise on every normal run, not just failures.
  evidence: edge-case-hunter and blind-hunter review of story 4.2 both flagged this; real and worth the owner's awareness, but the tradeoff is the explicit, deliberate result of this story's AC ("surfaces the shell's own error text... rather than hiding it") and already existed in production for `doctor.sh` before this story — not a defect introduced by this diff's logic, and fixing it would require redesigning the sourcing mechanism (e.g. capture-and-sanitize), beyond this story's scope of converging the sourcing form.

- source_spec: `skills/implementation-artifacts/spec-4-2-make-config-sourcing-consistent-across-the-scripts.md`
  summary: `doctor.sh` has no `selftest_case_broken_config` equivalent to the one added/strengthened on its five siblings, so its own AC2 behavior (surfacing a shell syntax error from a malformed `config/local.env`) remains self-test-unverified even though it's the reference form the other five now match.
  evidence: blind-hunter review of story 4.2 flagged this; a pre-existing test-coverage gap (predates this story), not required by AC3's literal text ("all six --check self-tests pass"), but real and would close by mirroring the pattern already used on the five siblings.
